...
type UConn struct {
	*Conn
	...
	handshakeStateInjected bool
	skipClientHello        bool
}
...
func (c *UConn) InjectHandshakeState(state PubClientHandshakeState) {
	c.HandshakeState = state
	c.handshakeStateInjected = true
	c.skipClientHello = true
}
...
func (c *UConn) handshakeContext(ctx context.Context) (ret error) {
	...
	// [uTLS section begins]
	if c.isClient {
		if c.skipClientHello {
			// HandshakeState has already been injected, and ClientHello was already sent via MSG_FASTOPEN.
...
func (uconn *UConn) computeAndUpdateOuterECHExtension(inner *clientHelloMsg, ech *echClientContext, useKey bool) error {
	...
	// https://github.com/SearchPilot/utls/blob/e430876b1d82fdf582efc57f3992d448e7ab3d8a/ech.go#L408
	var encapKey []byte
	if useKey {
